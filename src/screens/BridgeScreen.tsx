...
export default function BridgeScreen({ navigation, route }: Props) {
  ...
  const personA = route.params?.a ?? "none";
  const [selected, setSelected] = useState<OptionId>("vocal");
    ...
    <View style={styles.phoneFrame}>
      ...
      <ScrollView style={styles.contentScroll} contentContainerStyle={{ paddingBottom: insets.bottom + 12 }} showsVerticalScrollIndicator={false}>
        <View style={styles.heroArea}>
          ...
          <Text style={styles.progressLabel}>Bridge Setup · 2/2</Text>
          <Text style={styles.pageTitle}>Define Person B</Text>
          <Text style={styles.pageSub}>Configure the profile for their communication partner.</Text>
          ...
          {options.map((option) => {
            ...
            return (
              <Pressable
                ...
                style={[styles.modeCard, isSelected && styles.modeCardSelectedPink]}
              >
                <View style={[styles.selIndicator, isSelected && styles.selIndicatorPink]} />
      ...
      <View style={styles.stepActions}>
        <Pressable style={styles.btnBack} onPress={() => navigation.navigate("App")}>
          <Text style={styles.btnBackText}>← Back to Person A</Text>
        </Pressable>
        <Pressable style={styles.btnNext} onPress={() => navigation.navigate("Communicate", { a: personA, b: selected })}>
          <Text style={styles.btnNextText}>Start Bridge</Text>
      ...
      <BottomNav
        ...
        onSetupPress={() => navigation.navigate("Bridge", { a: personA })}
        onBridgePress={() => navigation.navigate("Communicate", { a: personA, b: selected })}
        onSettingsPress={() => navigation.navigate("Bridge", { a: personA })}
